...
def get_all_tasks_repo(user_id:str, limit: int, offset:int):
    with get_cur() as cur:
...
def get_task_by_name_repo(user_id:str, name: str):
    with get_cur() as cur:
        cur.execute("SELECT name, category, status, time_created, time_to_be_completed, time_completed FROM tasks WHERE user_id = %s AND name ILIKE %s", (user_id, f"%{name}%"))
        rows = cur.fetchall()
    print(rows)
    return rows
